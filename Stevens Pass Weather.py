...
def combine_two_dicts(dict1, dict2):
    ...
    for i in range(0,len(bigger)):
        if (bigger_key_list[i] in smaller_key_list) or (bigger_key_list[i] == "Today" and smaller_key_list[i] == "Tonight"):
...
def main():
    ...
    for forecast in noaa_twelve_hr_forecasts:
        ...
        if forecast.find('p', class_='temp-high') == None:
            day_title= str(forecast.find('p', class_='period-name').text)
            day_title = day_title.replace("Night","").replace(" ","")
    ...
    noaa_all_temp = combine_two_dicts(noaa_high_temps, noaa_low_temps)
    
    print("~~~~0~~~~\nWeather Forecast NOAA:\n")
    print_dict(noaa_all_temp)
